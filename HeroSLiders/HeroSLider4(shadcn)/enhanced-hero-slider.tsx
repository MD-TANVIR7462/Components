      ...
      <div className="max-w-7xl w-full mx-auto flex flex-col lg:flex-row items-center justify-between gap-12 relative">
        <div className="text-white space-y-8 lg:w-1/2 z-10">
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold leading-tight">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#b657ff] to-[#ff8989]">thrive</span> towards<br />
            your <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#b657ff] to-[#ff8989]">envision</span>.
          </h1>
          ...
          <ul className="space-y-4 text-lg md:text-xl">
            {services.map((service, index) => (
              ...
              >
                <service.icon className={`w-6 h-6 ${index === currentService ? 'text-[#b657ff]' : 'text-gray-400'}`} />
                {service.title}
              </li>
            ))}
          </ul>
            ...
            {services.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  index === currentService ? 'bg-white scale-125' : 'bg-gray-400'
                }`}
